...
import { 
  Plus, ChevronDown, RefreshCw, Camera, AlertTriangle, 
  Sparkles, ArrowRightLeft, Loader2, Mail, Lock, ArrowRight, Check, CheckCircle2,
  Wallet, UploadCloud, X
} from 'lucide-react';
...
/**
 * --- INTERNAL COMPONENTS (Modals & Screens) ---
 */
...
// 5. 用户资料弹窗 (仅用于显示详细信息，简单版本)
const UserProfileModal: React.FC<{ user: any; onClose: () => void; onLogout: () => void; }> = ({ user, onClose, onLogout }) => {
  // 注意：ProfilePage 组件已经是更完整的版本，这里的 Modal 主要是作为补充或快速查看
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-scaleIn">
        <div className="flex flex-col items-center mb-6">
          <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-4"><Wallet size={48} className="text-gray-400" /></div>
        ...
        <div className="space-y-3">
          <button onClick={onLogout} className="w-full py-3.5 bg-red-50 text-red-500 font-bold text-sm rounded-xl flex items-center justify-center gap-2 hover:bg-red-100 transition"><ArrowRight size={16} /> 退出登录</button>
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      // 移除 signInAnonymously，只使用邮箱或 CustomToken
    };
    initAuth();
  ...
  const handleDeleteTransactionRequest = (assetId: string, txId: string) => {
    // 专门用于从 AssetsPage 触发的删除请求
    if (!user) return;
    const asset = assets.find(a => a.id === assetId);
    if (!asset) return;
    
    // 这里我们直接复用更新逻辑，不弹窗确认了（或者你可以加个 window.confirm）
    if(window.confirm("确定删除这条记录吗？")) {
        const newHistory = asset.history.filter(tx => tx.id !== txId);
        updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'assets', assetId), { history: newHistory });
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" className="w-full bg-gray-50 border border-gray-200 rounded-xl h-12 px-3 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all" value={newAsset.date} onChange={(e) => setNewAsset({...newAsset, date: e.target.value})} />
                </div>
                <div>
                  <label className="block text-gray-500 text-xs font-bold mb-1.5">资产类型</label>
                  <select className="w-full bg-gray-50 border border-gray-200 rounded-xl h-12 px-3 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all appearance-none" value={newAsset.type} onChange={(e) => setNewAsset({...newAsset, type: e.target.value as AssetType})}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full bg-gray-50 border border-gray-200 rounded-xl h-12 px-3 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all appearance-none" value={newAsset.currency} onChange={(e) => setNewAsset({...newAsset, currency: e.target.value as Currency})}>
                ...
                <div>
                  ...
                  <input type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl h-12 px-3 text-lg font-bold text-gray-900 outline-none focus:ring-2 focus:ring-blue-500 transition-all" placeholder="0.00" value={newAsset.amount} onChange={(e) => setNewAsset({...newAsset, amount: e.target.value})} />
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl h-12 px-3 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all" placeholder="2.5" value={newAsset.yield} onChange={(e) => setNewAsset({...newAsset, yield: e.target.value})} />
                </div>
                <div className="flex-[2]">
                  <label className="block text-gray-500 text-xs font-bold mb-1.5">备注</label>
                  <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl h-12 px-3 text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all" placeholder="选填" value={newAsset.remark} onChange={(e) => setNewAsset({...newAsset, remark: e.target.value})} />
                </div>
              </div>
              
              <div className="flex gap-3 mt-8">
                <button onClick={() => setShowAddModal(false)} className="flex-1 py-3.5 rounded-xl bg-gray-100 text-gray-600 font-bold text-sm hover:bg-gray-200 transition-colors">取消</button>
                <button onClick={handleAddAsset} className="flex-1 py-3.5 rounded-xl bg-gray-900 text-white font-bold text-sm shadow-lg hover:bg-black transition-colors">确认</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-8 shadow-2xl">
            ...
            <div className="space-y-4 text-gray-600 text-sm leading-relaxed">
              <ul className="list-disc pl-5 space-y-2">
                ...
                <li><strong>混合货币支持</strong>：支持本金和收益使用不同的货币。</li>
